...
public sealed class HexTacticsModeSelectScreenView : HexTacticsUiGeneratedView
{
    ...
    public override void BuildDefaultHierarchy()
    {
        ...
        root.sizeDelta = new Vector2(520f, 290f);
        ...
        var title = HexTacticsUiFactory.CreateText(root, "Title", "六方向战棋原型", 30, TextAnchor.MiddleCenter, Color.white, FontStyle.Bold);
        ...
        var description = HexTacticsUiFactory.CreateText(root, "Description", "当前提供“对战 CPU”模式。先配置队伍，再进入同步指令结算战斗。", 18, TextAnchor.MiddleCenter, new Color(0.90f, 0.92f, 0.94f));
        HexTacticsUiFactory.AddLayoutElement(description.gameObject, preferredHeight: 72f);

        cpuButton = HexTacticsUiFactory.CreateButton(root, "CpuButton", "对战 CPU", new Color(0.22f, 0.56f, 0.55f, 0.96f), Color.white, out _);
        HexTacticsUiFactory.AddLayoutElement(cpuButton.gameObject, preferredHeight: 48f, preferredWidth: 220f);
    }
